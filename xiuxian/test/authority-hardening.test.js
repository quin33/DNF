...
async function startAuthorityServer() {
  ...
  const child = spawn(process.execPath, ['server.js'], {
    ...
    env: { ...process.env, PORT: String(port), ADMIN_PASSWORD: 'authority-test-password', TAVERN_LOAD_ENV: '0', AI_BASE_URL: '', AI_API_KEY: '', AI_MODEL: '' },
...
test('authoritative actions mutate only server-owned inventory and skill state', async t => {
  ...
  const created = await fetch(`${base}/api/character/create`, {
    ...
    body: JSON.stringify({ creation_request_id: `req-${Date.now()}-${Math.random()}`, name: `权威${Math.random().toString(36).slice(2, 9)}`, root: 'jin', gender: '男', pers: '重诺', items: ['iron_sword', 'condense_pill'] }),
  });
  ...
  assert.ok(boughtBody.character.skills.some(skill => skill.name === '吐纳诀'));
  current = boughtBody;
