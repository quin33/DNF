...
function buildUserMessage(b) {
  ...
  if (b.specialEvent) {
    ...
    lines.push(se && se.name
      ? `【特殊事件】本局触发异象“${se.name}”：${se.desc || ''}。副本整体凶险更甚（敌人修为更高、机关更险），但机缘亦更丰厚——剧情中应把这一异象贯穿始终，战利品与报酬可以更丰。`
      : '【特殊事件】本局触发异象：副本整体凶险更甚（敌人修为更高、机关更险），但机缘亦更丰厚——剧情中应烘托出异象骤生、凶机并现的氛围，战利品与报酬可以更丰。');
  }
...
/* AI 开本判定：由 AI 决定隐藏副本/特殊事件/突破试炼与敌人数量、修为；隐藏与特殊事件各有概率兜底，且可同时触发 */
const SETUP_PROMPT = `你是《问道仙坊》的开局推演师。根据副本背景、队伍修为与角色特质，决定本次探险的局势：
1. hidden：约一成概率触发隐藏凶地（首领盘踞、名称改变、凶险加倍）；不要默认 false。
2. specialEvent：约一成概率触发特殊异象（整体难度与机缘上调）；若氛围合适就触发，不必刻意回避；隐藏凶地同样可能出现特殊异象。
